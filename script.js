...
document.addEventListener('DOMContentLoaded', () => {
  ...
  planningBtn.onclick = (e) => {
    e.preventDefault(); // Empêche le lien de sauter à l'ancre #planning
    openPlanningModal();
  };
}
    if (randomBtn) randomBtn.onclick = () => openAnimeModal(animes[Math.floor(Math.random() * animes.length)]);

    // --- Modals Avertissement & Ecchi ---
    if (warningModal && localStorage.getItem('shinime_warning_ack') === '1') warningModal.style.display = 'none';
    const warningBtn = document.getElementById('shinime-warning-btn');
    if (warningBtn) warningBtn.onclick = () => {
	...
	if (pseudoDisplay && editPseudoBtn && avatarDisplay && bannerDisplay) {
        const displayPseudo = () => {
    ...
    // --- Boutons de suppression (Profil) ---
    const clearWatchlistBtn = document.getElementById('clear-watchlist-btn');
    const clearReprendreBtn = document.getElementById('clear-reprendre-btn');
    const clearFavorisBtn = document.getElementById('clear-favoris-btn');

    if (clearWatchlistBtn) {
        clearWatchlistBtn.onclick = () => {
            if (confirm("Êtes-vous sûr de vouloir vider votre Watchlist ?")) {
                localStorage.removeItem('shinime_watchlist');
                displayAllGrids(); // Rafraîchit l'affichage
            }
        };
    }

    if (clearReprendreBtn) {
        clearReprendreBtn.onclick = () => {
            if (confirm("Êtes-vous sûr de vouloir vider votre liste 'Reprendre' ?")) {
    ...
    if (clearFavorisBtn) {
        clearFavorisBtn.onclick = () => {
            if (confirm("Êtes-vous sûr de vouloir vider vos Favoris ?")) {
                localStorage.removeItem('animeFavorites');
                displayAllGrids(); // Rafraîchit l'affichage et les stats
            }
        };
    }
		};
        editPseudoBtn.onclick = () => {
            const currentPseudo = localStorage.getItem('shinime_pseudo') || '';
            const newPseudo = prompt("Quel est votre nouveau pseudo ?", currentPseudo);
            if (newPseudo !== null) {
        ...
        const displayAvatar = () => {
		    ...
		    // ▼▼▼ AJOUTE CE BLOC POUR LA BANNIÈRE ▼▼▼
    const displayBanner = () => {
        ...
        if (bannerUrl) {
          bannerDisplay.style.backgroundImage = `url('${bannerUrl}')`;
        }
    };

    bannerDisplay.onclick = () => openBannerModal();
    // ▲▲▲ FIN DU BLOC POUR LA BANNIÈRE ▲▲▲
