...
function getReprendreList() {
	...
	return animes.filter(a =>
		...
		progress[a.id] < a.episodeUrls.length
	);
}


// (Pour la lisibilité, je ne recopie pas tout le tableau animes ici, mais garde-le tel quel dans ton fichier.)

// === Sélection des grilles ===
const nouveauteGrid = document.getElementById('nouveauteGrid');
const catalogueGrid = document.getElementById('catalogueGrid');
const favorisGrid = document.getElementById('favorisGrid');
const avenirGrid = document.getElementById('avenirGrid');
const reprendreGrid = document.getElementById('reprendreGrid');
const searchInput = document.getElementById('searchInput');
const searchResults = document.getElementById('searchResults');
const ecchiGrid = document.getElementById('ecchiGrid');
const ecchiList = animes.filter(a => Array.isArray(a.genres) && a.genres.some(g => g.toLowerCase() === "ecchi"));
displayAnimes(ecchiList, ecchiGrid);
...
function populateLetterFilter() {
	...
	animes.forEach(a => {
		if (a.title && typeof a.title === "string") {
			letterSet.add(a.title[0].toUpperCase());
		}
	});
	const letters = Array.from(letterSet).sort();
	const letterSelect = document.getElementById('filter-letter');
	letterSelect.innerHTML = '<option value="">Lettre</option>' + letters.map(l => `<option value="${l}">${l}</option>`).join('');
}
...
function populateGenreCheckboxes() {
	...
	animes.forEach(a => {
		if (Array.isArray(a.genres)) a.genres.forEach(g => genresSet.add(g));
	});
	const genres = Array.from(genresSet).sort((a, b) => a.localeCompare(b, 'fr', {
		sensitivity: 'base'
	}));
...
document.getElementById('filter-genres').addEventListener('change', filterCatalogue);

function filterCatalogue() {
	const letter = document.getElementById('filter-letter').value;
	...
	if (letter) {
		filtered = filtered.filter(a => a.title.toUpperCase().startsWith(letter));
	}
	if (checkedGenres.length > 0) {
		filtered = filtered.filter(a =>
			a.genres && checkedGenres.every(g => a.genres.includes(g))
		);
	}
	if (year) {
		filtered = filtered.filter(a => a.year && a.year.toString() === year);
	}
	displayAnimes(filtered, catalogueGrid);
}
...
function populateFilters() {
	...
	yearSelect.innerHTML = '<option value="">Année</option>' + years.map(y => `<option value="${y}">${y}</option>`).join('');
}
populateFilters();
document.getElementById('filter-letter').addEventListener('change', filterCatalogue);
document.getElementById('filter-year').addEventListener('change', filterCatalogue);
document.getElementById('reset-filters').addEventListener('click', function() {
	...
	displayAnimes(catalogue, catalogueGrid);
});

// === Favoris ===
function getFavorites() {
	const favs = localStorage.getItem('animeFavorites');
...
function displayAnimes(list, grid) {
	const favorites = getFavorites();
	grid.innerHTML = '';
	if (list.length === 0) {
		grid.innerHTML = `<p>Aucun animé trouvé 😕</p>`;
		return;
	}
	list.forEach(anime => {
		...
		let episodeAReprendre = null;
		if (grid === reprendreGrid) {
			const progress = JSON.parse(localStorage.getItem('animeProgress') || '{}');
			if (progress[anime.id] !== undefined) {
				episodeAReprendre = progress[anime.id] + 1;
			}
		}
		card.innerHTML = `
      ...
      <div class="anime-info">
        ...
        ${episodeAReprendre ? `<div style="font-size:0.95em;color:#61dafb;font-weight:600;">Reprendre à l'épisode ${episodeAReprendre}</div>` : ''}
        <button
          class="favorite-btn ${favorites.has(anime.id) ? 'favorited' : ''}"
          title="Ajouter aux favoris"
          aria-label="${favorites.has(anime.id) ? 'Retirer des favoris' : 'Ajouter aux favoris'}"
        >♥</button>
        ${episodeAReprendre ? `
          <button class="remove-reprendre-btn" style="margin-top:6px;font-size:0.9em;background:#ff4081;color:#fff;border:none;border-radius:4px;padding:2px 8px;cursor:pointer;">
            Retirer
          </button>
        ` : ''}
      </div>
    `;
		if (grid === reprendreGrid) {
			const removeBtn = card.querySelector('.remove-reprendre-btn');
			if (removeBtn) {
				removeBtn.addEventListener('click', (e) => {
					e.stopPropagation();
					let progress = JSON.parse(localStorage.getItem('animeProgress') || '{}');
					delete progress[anime.id];
					localStorage.setItem('animeProgress', JSON.stringify(progress));
					displayAnimes(getReprendreList(), reprendreGrid);
				});
			}
		}
		...
		favBtn.addEventListener('click', (e) => {
			...
			displayAnimes(avenir, avenirGrid);
			displayAnimes(nouveautes, nouveauteGrid);
			displayAnimes(catalogue, catalogueGrid);
			displayFavoris();
			displayAnimes(getReprendreList(), reprendreGrid);
		});
...
function displayFavoris() {
	const favorites = getFavorites();
	const favorisList = animes.filter(a => favorites.has(a.id));
	displayAnimes(favorisList, favorisGrid);
}

// === Gestion des grilles ===

// 1. Définissez ICI les IDs des animés que vous voulez dans "Nouveautés" et "À venir"
const nouveauteIds = [2, 6, 8, 18, 23, 27]; // <-- Mettez les IDs des nouveautés ici
const avenirIds = [20, 42]; // <-- Mettez les IDs des animés à venir ici
const ecchiIds = []; // <-- Mettez les IDs des animés ecchi ici
// 2. Le script trouve les animés correspondants à ces IDs
...
// 3. Le reste du code fonctionne sans changement
const catalogue = animes
  .filter(a => ![...avenirIds, ...nouveauteIds, ...ecchiIds].includes(a.id))
...
function openAnimeModal(anime) {
	...
	if (Array.isArray(anime.episodeUrls)) {
		anime.episodeUrls.forEach((url, idx) => {
			...
			if (idx === 0) epBtn.classList.add('active');
			epBtn.addEventListener('click', () => {
				iframe.src = url;
				[...modalLeft.children].forEach(b => b.classList.remove('active'));
			...
			modalLeft.appendChild(epBtn);
		});
	}

	// Right - image + description + recommandations
	...
	modalRight.appendChild(desc);

	// Recommandations personnalisées
	const recos = getRecommendations(anime);
	if (recos.length > 0) {
		const recoTitle = document.createElement('div');
		recoTitle.textContent = "Tu as aimé cet animé ? Essaie aussi :";
		recoTitle.style.marginTop = "1rem";
		recoTitle.style.fontWeight = "bold";
		recoTitle.style.color = "#00ffe5";
		modalRight.appendChild(recoTitle);
		recos.forEach(r => {
			const recoBtn = document.createElement('button');
			recoBtn.textContent = r.title;
			recoBtn.style.margin = "0.3rem 0";
			recoBtn.style.background = "#23232b";
			recoBtn.style.color = "#00ffe5";
			recoBtn.style.border = "none";
			recoBtn.style.borderRadius = "6px";
			recoBtn.style.padding = "0.3rem 0.7rem";
			recoBtn.style.cursor = "pointer";
			recoBtn.onclick = () => {
				document.body.removeChild(overlay);
				openAnimeModal(r);
			};
			modalRight.appendChild(recoBtn);
		});
	}

	// Historique
	let viewed = JSON.parse(localStorage.getItem('viewedAnimes') || '[]');
	if (!viewed.includes(anime.id)) {
		viewed.push(anime.id);
		localStorage.setItem('viewedAnimes', JSON.stringify(viewed));
	}
...
// Recommandations personnalisées
function getRecommendations(currentAnime) {
	const viewed = JSON.parse(localStorage.getItem('viewedAnimes') || '[]');
	return animes.filter(a =>
		a.id !== currentAnime.id &&
		Array.isArray(a.genres) &&
		Array.isArray(currentAnime.genres) &&
		a.genres.some(g => currentAnime.genres.includes(g)) &&
		!viewed.includes(a.id)
	).slice(0, 3);
}

// === Planning manuel ===
const planningData = {
	"Lundi": ["Rick et Morty", "Food Court de Mata Ashita", "Summer Pockets", "Kijin Gentoushou", "Sakamoto Days", "Wata-Nare", "Grand Blue"],
	"Mardi": ["Rent a Girldfriend", "A Couple of Cuckoos", "Mattaku Saikin no Tantei to Kitara", "Sentai Daishikkaku"],
	"Mercredi": ["Clevatess", "The Rising of the Shield Hero", "Reborn as a Vending Machine", "I Was Reincarnated as a 7Th Prince", "Mikadono Sanshimai Wa Angai Choroi", "Onmyou Kaiten Re:Birth", "New Saga"],
	"Jeudi": ["Karaoke Iko", "Dr.Stone", "Dandadan", "The Water Magician"],
	"Vendredi": ["Dragon Raja", "Arknights", "Tougen Anki", "Yofukashi no Uta", "Silent Witch", ""],
	"Samedi": ["Lord of Mysteries", "The All - Devouring Whale", "Yaiba", "Detective Conan", "Kaiju No. 8"],
	"Dimanche": ["To be Hero X", "Witch Watch", "Dekin no Mogura", "Mynoghra", "Gachiakuta", " Hotel Inhumans", "One Piece EggHead", "Black Butler"]
};
...
document.getElementById('planningBtn').addEventListener('click', () => {
	...
	const planningWrapper = document.createElement('div');
	planningWrapper.className = 'planning-table-wrapper';
	const planningTable = document.createElement('table');
	planningTable.className = 'planning-table';
	planningTable.style.width = '100%';
	planningTable.style.borderCollapse = 'collapse';
	planningTable.style.margin = '1.5rem 0 1rem 0';
	planningTable.style.background = 'rgba(30,30,30,0.95)';
	planningTable.style.borderRadius = '8px';
	planningTable.style.overflow = 'hidden';
	const headerRow = document.createElement('tr');
	days.forEach(day => {
		const th = document.createElement('th');
		th.textContent = day;
		th.style.padding = '0.7rem 0.3rem';
		th.style.fontWeight = 'bold';
		th.style.fontSize = '1.1rem';
		th.style.color = '#61dafb';
		th.style.background = '#181818';
		th.style.border = '1px solid #292929';
		headerRow.appendChild(th);
	});
	planningTable.appendChild(headerRow);
	const contentRow = document.createElement('tr');
	days.forEach(day => {
		const td = document.createElement('td');
		td.style.height = '60px';
		td.style.border = '1px solid #292929';
		td.style.textAlign = 'center';
		td.style.color = '#eee';
		const animesOfDay = planningData[day] || [];
		animesOfDay.forEach(title => {
			const div = document.createElement('div');
			div.textContent = title;
			div.className = 'planning-anime';
			td.appendChild(div);
		});
		contentRow.appendChild(td);
	});
	planningTable.appendChild(contentRow);
	planningWrapper.appendChild(planningTable);
	modal.appendChild(closeBtn);
	modal.appendChild(planningWrapper);
	overlay.appendChild(modal);
	document.body.appendChild(overlay);
});
...
function displayHistorySidebar() {
	...
	ordered.forEach(anime => {
		...
		div.innerHTML = `
      <img src="${anime.img}" alt="${anime.title}" />
      <span class="history-anime-title">${anime.title}</span>
      <button class="history-anime-remove" title="Retirer de l'historique" aria-label="Retirer de l'historique">✖</button>
    `;
		div.onclick = () => openAnimeModal(anime);
		const removeBtn = div.querySelector('.history-anime-remove');
		removeBtn.onclick = (e) => {
			e.stopPropagation();
			let history = JSON.parse(localStorage.getItem('animeHistory') || '[]');
			history = history.filter(id => id !== anime.id);
			localStorage.setItem('animeHistory', JSON.stringify(history));
			displayHistorySidebar();
		};
...
window.addEventListener('DOMContentLoaded', () => {
	const saved = localStorage.getItem('shinime_theme');
	if (saved === 'light') setTheme('light');
});

document.getElementById('random-anime-btn').onclick = function() {
	const random = animes[Math.floor(Math.random() * animes.length)];
	openAnimeModal(random);
};
window.addEventListener('DOMContentLoaded', function() {
	if (localStorage.getItem('shinime_warning_ack') === '1') {
		document.getElementById('warning-modal').style.display = 'none';
	}
	document.getElementById('shinime-warning-btn').onclick = function() {
		if (document.getElementById('shinime-hide-warning').checked) {
			localStorage.setItem('shinime_warning_ack', '1');
		}
		document.getElementById('warning-modal').style.display = 'none';
	};
});
...
document.getElementById('ecchi-age-btn').onclick = function() {
	document.getElementById('ecchi-age-modal').style.display = 'flex';
};
document.getElementById('ecchi-age-confirm').onclick = function() {
	ecchiAgeVerified = true;
	document.getElementById('ecchi-age-modal').style.display = 'none';
	updateEcchiOverlay();
};
document.getElementById('ecchi-age-cancel').onclick = function() {
	document.getElementById('ecchi-age-modal').style.display = 'none';
};
...
// Bloque certaines combinaisons de touches
document.addEventListener("keydown", function(e) {
	...
	// Ctrl + Shift + C (Inspecteur Élément)
