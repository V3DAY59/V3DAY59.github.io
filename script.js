...
function populateGenreCheckboxes() {
  ...
  const genres = Array.from(genresSet).sort((a, b) => a.localeCompare(b, 'fr', { sensitivity: 'base' }));
...
const avenir = animes.filter(a => avenirIds.includes(a.id));
const nouveauteIds = [1, 4, 3, 5, 9, 10, 14, 24, 30];
const nouveautes = animes.filter(a => nouveauteIds.includes(a.id));
const catalogue = animes
  .filter(a => ![...avenirIds, ...nouveauteIds].includes(a.id))
...
searchInput.addEventListener('input', () => {
  ...
  const filtered = animes.filter(a => a.title.toLowerCase().includes(query));
